function outs = SPIFEE_Single(name,data,feats,hours, lenPulse)
close all
%Means
means = mean(feats, 2)

%Treatment-Wide features
peakPer = length(feats) / length(unique(feats(9,:)))
NumResponding = length(unique(feats(9,:)))
percentResponse = (length(unique(feats(9,:))) / width(data)) * 100;

outs = [means;peakPer;NumResponding;percentResponse]
...
name1 = append(name, "_AvgTrace")
savefig(avgTraceFig, name1)



%Peaks per cell in general
...
name2 = append(name, "_ClusterEval")
exportgraphics(OptimalClusterFig, append(name2,'.pdf'))
...
for i = 1:BestK
plot(data(:,idx == i))
[NumLength, num] = size(data(:,idx == i))
cellNums = find(idx == i)
peaks = find(any(feats(9,:) == cellNums))
currCluster = mean(feats(:,peaks),2)
peakPer = length(peaks) / length(unique(feats(9,peaks)))
NumResponding = length(unique(feats(9,peaks)))
percentResponse = 1; %This feature is more for treatments as a whole, but for the sake of continuity is included here
currCluster = [currCluster;peakPer;NumResponding;percentResponse]
clustMeans = [clustMeans, currCluster]
...
name3 = append(name, "_ClusterAssigns")
savefig(ClusterAssignFig, name3)
...
% results.MeanPerCluster = ClusterMeans
...
name4 = append(name, "_AvgClusterTrace")
savefig(avgClusterTraceFig, name4)
