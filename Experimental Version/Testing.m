...
P53_HighFreqFeat = PeakProcessingUpdated(P53_HighFreq,73,24,57,"p53")
%% example reisdual findchangepts
% close all
% temp1 = smoothdata(P53_LowAmp(:,1), 'gaussian', 9)
% t = linspace(0,24,73)
% [pks,locs,w,p] = findpeaks(temp1,t,"MinPeakHeight", 100, "MinPeakProminence", 20, "MinPeakDistance", 1, "MinPeakWidth",.75);
% [ipt] = findchangepts(temp1, "MaxNumChanges", (length(pks) * 2 + 1), 'Statistic', 'linear')
% findchangepts(temp1, "MaxNumChanges", (length(pks) * 2 + 1), 'Statistic', 'linear')
% 
% figure()
% findpeaks(temp1,t,"MinPeakHeight", 100, "MinPeakProminence", 20, "MinPeakDistance", 1, "MinPeakWidth",.75, 'Annotate','extents');
% figure()
% 
% dtemp1 = diff(temp1)
% plot(dtemp1)
