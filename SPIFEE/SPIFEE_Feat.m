function [Details, FiltData,Features] = SPIFEE_Feat(Data, Hours, lenPulse, idx, params)
...
%Remove minpeakheight warning. TO DO / warn in some other way at end of
%script.
...
if params.Filt == "Default Gaussian" || params.Filt == ""
...
for i = 1:NumCells
   ...
   %Calculate TemporalFeatures
       tramps = zeros(1, numPeaks);
       drops  = zeros(1, numPeaks);
       AUC    = zeros(1, numPeaks);
       timePoints = 1:Points;
   ...
   %It has its own warning, but we can't know aprior the amount of peaks.
        for k=1:numPeaks
            Features(1,j) = pks(k);
            Features(2,j) = locs(k);
